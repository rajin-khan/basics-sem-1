...
void addProd(int valid_entry) {
  ...
  for (int i = 0; i < 100; i++) {
    fscanf(fp, "%s%s%d", inventory[i].prod_name, inventory[i].type,
           &inventory[i].price);
...
void viewProd() {
  ...
  while (1) {
    fscanf(fp, "%s%s%d", view_inventory[count].prod_name, view_inventory[count].type,&view_inventory[count].price);
    count++;
    if (feof(fp))
      break;
  }
...
void searchProd() {
  ...
  while (1) {
    fscanf(fp, "%s%s%d", inventory[count].prod_name, inventory[count].type,
...
void deletProd() {
  ...
  while (1) {
    fscanf(fp, "%s%s%d", inventory[count].prod_name, inventory[count].type, &inventory[count].price);
...
void main_menu() {
  ...
  printf("\t# UNIX GROCERY MANAGEMENT SYSTEM #\n\n");
  ...
  printf("Choose any of the options above (enter the corresponding number):");
  scanf("%d", &main_no);
  fflush(stdin);
  ...
  case 1:
    ...
    printf("Enter number valid inputs(must be less than 100):");
    scanf("%d", &valid_input);
    fflush(stdin);
